...
The 1.3–1.6× gap between Triton direct and PyTorch (cuDNN) is algorithmic: cuDNN selects Winograd F(2,3) for K=3 stride-1, reducing the multiply count ~2.25× vs direct convolution. `winograd_conv2d` implements the same algorithm; benchmark numbers will be updated after the Colab run.
